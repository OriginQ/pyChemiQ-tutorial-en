...
Usually, the ansatz constructed based on hardware characteristics consists of many layers, and each layer's quantum circuit consists of two parts: firstly, single-qubit rotation gates (RZ-RX-RZ) on each qubit; secondly, controlled logic gates (controlled-RY gates) formed by two adjacent qubits entangled with each other. The single-layer quantum circuit is shown in the following figure:
